...
export default function InkReveal({
  ...
  const loop = useCallback(() => {
    ...
    if (stamps.length) {
      // Self-referencing RAF loop: `loop` is only read once this callback
      // actually runs (asynchronously, after the const assignment below
      // completes), so the closure is safe. This project doesn't enable
      // the React Compiler, so the rule's static warning doesn't apply.
